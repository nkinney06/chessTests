...
int evalTwo()
{
	...
	// if ( TIME_BONUS_TWO > 0 ) {
		...
		// for ( i = 0 ; i < 64 ; i++ ) {
			// if ( squareCount[i] > 0 ) {
				// if ( squares[i] < 8 )
					// whitescore += TIME_BONUS_TWO * squareCount[i];
				// else
					// blackscore += TIME_BONUS_TWO * squareCount[i];
			// }
		// }
	// }
	...
	whitescore += BONUS_MOVE_OPTIONS_TWO * legalMovesWhite;
	blackscore += BONUS_MOVE_OPTIONS_TWO * legalMovesBlack;
		...
		if ( rank[square] < 5 )
			if ( !(OUTPOST_WHITE[square] & BLACK_PAWNS) )
				if ( blackpawnattacks[square] & WHITE_PAWNS )
					whitescore += GOOD_KNIGHT_BONUS_TWO;
	...
	if (WHITE_BISHOPS)
		if ((WHITE_BISHOPS & WHITE_SQUARES) && (WHITE_BISHOPS & BLACK_SQUARES))
			whitescore += BONUS_BISHOP_PAIR_TWO;
		...
		if ( bitset[square] & 0x40a040000000000 )
			if ( bitcount( WHITE_PAWNS & 0x100800000000 ) > 1 )
				if ( bitcount( BLACK_PAWNS & 0x1008000000 ) >= 1 )
					whitescore -= BAD_BISHOP_PENALTY_TWO;
	...
	while (temp) {
		...
		if (FILEMASK[square] & whitepassedpawns)
			if ((unsigned int) square > firstone(FILEMASK[square] & whitepassedpawns))
				whitescore += BONUS_ROOK_BEHIND_PASSED_PAWN_TWO;

		if ( !(FILEMASK[square] & WHITE_PAWNS) ) {
		
		// rook on semi open file
			if ( !(FILEMASK[square] & BLACK_PAWNS) )
				whitescore += ROOK_SEMI_OPEN_FILE_BONUS_TWO;
		
		// rook on open file
			else
				whitescore += ROOK_OPEN_FILE_BONUS_TWO;
		}

		// rook on seventh bonus
		if (RANKS[square] == 2)
			whitescore += ROOK_ON_SEVENTH_BONUS_TWO;
	...
	while (temp) {
		...
		if (!(PASSED_BLACK[square] & WHITE_PAWNS)) {  
			if ( !( ( PASSED_BLACK[square] & FILEMASK[square] ) & BLACK_PAWNS ) ) {
				blackscore += BONUS_PASSED_PAWN_TWO;
		...
		if ( rank[square] > 2 )
			if ( !(OUTPOST_BLACK[square] & WHITE_PAWNS) ) 
				if ( whitepawnattacks[square] & BLACK_PAWNS ) 
					blackscore += GOOD_KNIGHT_BONUS_TWO;
	...
	if (BLACK_BISHOPS)
		if ((BLACK_BISHOPS & WHITE_SQUARES) && (BLACK_BISHOPS & BLACK_SQUARES))
			blackscore += BONUS_BISHOP_PAIR_TWO; 
		...
		if ( bitset[square] & 0x40a04 ) 
			if ( bitcount( BLACK_PAWNS & 0x8100000 ) > 1 ) 
				if ( bitcount( WHITE_PAWNS & 0x810000000 ) >= 1 ) 
					blackscore -= BAD_BISHOP_PENALTY_TWO;
	...
	while (temp) {
		...
		if (FILEMASK[square] & blackpassedpawns)
			if ((unsigned int) square < lastone(FILEMASK[square] & blackpassedpawns))
				blackscore += BONUS_ROOK_BEHIND_PASSED_PAWN_TWO;
		...
		// rook on semi open file
			if ( !(FILEMASK[square] & WHITE_PAWNS) )
				blackscore += ROOK_SEMI_OPEN_FILE_BONUS_TWO;
				
		// rook on open file
			else
				blackscore += ROOK_OPEN_FILE_BONUS_TWO;
		}

		// rook on seventh
		if (RANKS[square] == 7)
			blackscore += ROOK_ON_SEVENTH_BONUS_TWO;
